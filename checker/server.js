...
const MAX_PAGES_LIMIT = Math.max(1, Math.min(200, Number(process.env.MAX_PAGES_LIMIT) || 50));
...
function outputPath(name) {
  return path.join(getOutputDirectory(), name);
}
...
const state = {
  ...
  error: null,
  logs: [],
  reportReady: fs.existsSync(outputPath('report.html'))
};
...
function addLog(text, type = 'info') {
  ...
  if (state.logs.length > 700) state.logs.splice(0, state.logs.length - 700);
}

function readProgress() {
  try {
    return JSON.parse(fs.readFileSync(outputPath('progress.json'), 'utf8'));
  } catch {
    return null;
  }
}
...
function publicState() {
  const reportExists = fs.existsSync(outputPath('report.html'));
  const progress = readProgress();
  return {
    ...state,
    reportReady: reportExists,
    reportPartial: Boolean(reportExists && state.running),
    reportUrl: reportExists ? `/output/report.html?v=${Date.now()}` : null,
    progress
  };
}
...
function parseOrigins(value) {
  ...
  for (const entry of entries) {
    ...
    catch { throw new Error(`Origin tambahan tidak valid: ${trimmed}`); }
...
function serveFile(res, filePath) {
  ...
  res.writeHead(200, {
    ...
    'Cache-Control': 'no-store',
    'X-Content-Type-Options': 'nosniff',
    'Content-Security-Policy': "default-src 'self' 'unsafe-inline' data: blob: https:; frame-ancestors 'self';"
  });
...
async function startScan(options) {
  ...
  const maxPages = Math.max(1, Math.min(MAX_PAGES_LIMIT, Number(options.maxPages) || Number(base.maxPages) || 30));
  ...
  const runtimeConfig = {
    ...
    maxDynamicMenuClicks: Math.max(0, Math.min(20, Number(options.maxDynamicMenuClicks) || Number(base.maxDynamicMenuClicks) || 8)),
  ...
  addLog(`Klik menu dinamis maksimal: ${runtimeConfig.maxDynamicMenuClicks}`);
  ...
  const child = spawn(process.execPath, [path.join(ROOT, 'scan.js'), RUNTIME_CONFIG_PATH], {
    ...
    env: {
      ...process.env,
      FORCE_COLOR: '0',
      NODE_OPTIONS: process.env.NODE_OPTIONS || '--max-old-space-size=320'
    }
  });
  ...
  child.on('error', (error) => {
    ...
    state.reportReady = fs.existsSync(outputPath('report.html'));
  ...
  child.on('close', (code, signal) => {
    ...
    if (code === 0) {
      addLog('Pemeriksaan selesai. Laporan HTML sudah tersedia.', 'success');
      return;
    }

    const suffix = signal ? ` (signal ${signal})` : '';
    state.error = `Scanner berhenti dengan kode ${code ?? 'tidak diketahui'}${suffix}.`;
    if (signal === 'SIGKILL') {
      state.error += ' Kemungkinan proses kehabisan RAM pada hosting.';
    }
    addLog(state.error, 'error');
    if (state.reportReady) addLog('Laporan sementara tetap dapat dibuka.', 'warning');
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && requestUrl.pathname === '/api/config') {
    const config = readBaseConfig();
    sendJson(res, 200, {
      maxPages: Math.min(config.maxPages || 30, MAX_PAGES_LIMIT),
      maxPagesLimit: MAX_PAGES_LIMIT,
      maxDynamicMenuClicks: config.maxDynamicMenuClicks || 8,
      autoScroll: config.autoScroll !== false,
      saveScreenshots: config.saveScreenshots === true,
